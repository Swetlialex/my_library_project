...
def add_book_interactively(library):
    ...
    purchase_date = input("Дата на покупка (дд-мм-гггг): ")
...
def filter_books_interactively(library):
    ...
    genres = input("Жанрове (проза или поезия / разделени със запетая, или празно): ").split(",") if input("Филтрирай по жанр? (да/не): ").lower() == "да" else None
    status = input("Статус на четене (прочетена/непрочетена/в процес на четене): ") if input("Филтрирай по статус? (да/не): ").lower() == "да" else None
    year_range = None
